...
def load_data():
    ...
    hourly = pd.read_csv("../proj_data/hourly_weather_tidy_streamlit.csv", parse_dates=['utc_timestamp'])
    ...
    rain = pd.read_csv("../proj_data/precipitation_tidy_streamlit.csv", parse_dates=['year'])
...
if trends_s.empty:
    ...
    fig_trends.update_layout(
        title_font_size=16,
        xaxis=dict(showgrid=True, gridcolor='darkgrey', linecolor='black'),
...
col2.metric("🔴 Hottest for its Radiation", hottest_for_rad,
            delta=f"{combined_q26_s['anomaly'].min():+.2f}",
            delta_color='inverse')
...
col1.metric("🎯 Most Average Country",  distances_q28.iloc[0]['country'],
            f"distance: {distances_q28.iloc[0]['distance']:.3f}")
col2.metric("🌟 Most Extreme Country",  distances_q28.iloc[-1]['country'],
            f"distance: {distances_q28.iloc[-1]['distance']:.3f}")
